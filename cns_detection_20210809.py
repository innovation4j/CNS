...
model = tensorflow.keras.models.load_model('keras_model_20210718.h5')
...
def validationInput(inputArr):
    ...
    imgLocBox    = f"image_storage/{equipmentId}/{settingSource[0]}/box/"
    imgLocFind   = f"image_storage/{equipmentId}/{settingSource[0]}/find/"
    imgLocBoxRef = f"image_storage/{equipmentId}/{settingSource[0]}_ref/box/"
    imgLocFindRef= f"image_storage/{equipmentId}/{settingSource[0]}_ref/find/"
    imgLocDetect = f"image_storage/{equipmentId}/detection/"
...
def objectProcessing(objectFr, objectTo, time_0, time_10):
    ...
    if ( objectFr[0]-150 < objectTo[0] < objectFr[0]+150 ):
...
def resetStatus():
    ...
    if( maskFlag == True and thresholdTime != None ):
        now = datetime.datetime.now()
        if ((now - thresholdTime).seconds/3600 > 1 ):
